        ...
        if token in ["x7dao", "x7r"]:
            ...
            prices = {
                t: dextools.get_pool_price(
                    tokens.get_tokens()[t.upper()][chain].pairs, chain
                )
            ...
            [
                [
                    InlineKeyboardButton(
                        ...
                        url=urls.XCHANGE_BUY(chain_info.id, ca.X7R(chain)),
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="X7DAO - Governance Token",
                        url=urls.XCHANGE_BUY(chain_info.id, ca.X7DAO(chain)),
                    )
                ],
            ]
        ),
...
async def feeto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tx_filter = [
        d
        for d in tx["result"]
        if ca.LIQUIDITY_TREASURY(chain).lower() in d["to"].lower()
        and any(
...
async def splitters_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    eco_address = ca.ECO_SPLITTER(chain)
    ...
    if chain == "eth":
        treasury_address = ca.TREASURY_SPLITTER(chain)
